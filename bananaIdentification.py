...
# Notes - Some else statements are useless however make code more readable
...
def main():
    ...
    for image_path in os.listdir('inputImages'):
        print(image_path)
        image = cv2.imread(os.path.join('inputImages', image_path))
        cv2.imwrite(os.path.join(path, image_path), identifyBanana('template', image))
        cv2.waitKey(0)
